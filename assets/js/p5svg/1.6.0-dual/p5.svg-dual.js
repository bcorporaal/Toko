    ...
    ***************************************************************************** */
    ...
    var Context = (function () {
        ...
        Context.prototype.__applyTransformation = function (element, matrix) {
            ...
            // For SVG, we don't want pixel density scaling
            // If the matrix has scaling (a or d != 1), reset it to identity
            if (a !== 1 || d !== 1) {
                element.setAttribute('transform', `matrix(1 0 0 1 ${e} ${f})`);
            } else {
    ...
    function RendererSVG (p5, isV2) {
            ...
            var pInstProxy = new Proxy(pInst, {
                get: function (target, prop) {
                    if (prop === '_pixelDensity') {
                        return 1; // SVG doesn't need pixel density scaling
                    }
                    return target[prop];
                },
                set: function (target, prop, value) {
                    if (prop === '_pixelDensity') {
                        return true; // Ignore pixel density changes
                    }
                    target[prop] = value;
                    return true;
                }
            });
            // Use appropriate constructor based on version
            if (isV2) {
                // p5.js v2: Create temporary canvas for Renderer2D constructor
                var tempCanvas = document.createElement('canvas');
                tempCanvas.width = elt.width;
                tempCanvas.height = elt.height;
                tempCanvas.id = elt.id;
                tempCanvas.className = elt.className;
                
                var renderer2D = new p5.Renderer2D(pInstProxy, elt.width, elt.height, isMainCanvas, tempCanvas);
                
                // Store reference to renderer2D for accessing properties that might not be copied
                this._tempRenderer2D = renderer2D;
                
                Object.assign(this, renderer2D);
                
                // Initialize shapeBuilder if it doesn't exist (it should be initialized in Renderer constructor)
                // But if Object.assign didn't copy it, we need to initialize it ourselves
                if (!this.shapeBuilder) {
                    // Try to get it from renderer2D first
                    if (renderer2D.shapeBuilder) {
                        this.shapeBuilder = renderer2D.shapeBuilder;
                    } else {
                        // Initialize it ourselves - we need to access ShapeBuilder class
                        // Since ShapeBuilder might not be exported, we'll try to create it
                        // by accessing it through the renderer's prototype or by using the same pattern
                        try {
                            // Try to access ShapeBuilder from p5 namespace
                            if (p5.ShapeBuilder) {
                                this.shapeBuilder = new p5.ShapeBuilder(this);
                            } else {
                                // Fallback: create a minimal shapeBuilder-like object
                                // We'll need to implement the essential methods
                                this.shapeBuilder = {
                                    renderer: this,
                                    shapeMode: p5.PATH,
                                    geometry: null,
                                    constructFromContours: function(shape, contours) {
                                        // This will be implemented in drawShape
                                    }
                                };
                            }
                        } catch (e) {
                            // Failed to initialize shapeBuilder - drawShape will handle this
                        }
                    }
                }
                
                // Copy buffers if needed
                if (renderer2D.buffers && !this.buffers) {
                    this.buffers = renderer2D.buffers;
                }
                
                // Replace with our SVG element
                this.elt = elt;
                this.canvas = elt;
                this.drawingContext = elt.ctx;
            } else {
                // p5.js v1: use call method
                Object.assign(this, p5.Renderer2D.call(this, elt, pInstProxy, isMainCanvas));
            }
            this.isSVG = true;
            this.svg = svg;
            
            // For p5v2, ensure _pInst._renderer points back to this SVG renderer
            // This is critical for beginShape/endShape to work correctly
            if (isV2 && pInstProxy) {
                pInstProxy._renderer = this;
            }
        ...
        RendererSVG.prototype._applyDefaults = function () {
            ...
            // Override pixel density for SVG renderer
        ...
        RendererSVG.prototype.resize = function (w, h) {
            ...
            // For SVG, we don't want pixel density scaling
            // Set SVG dimensions to logical size
            this.svg.setAttribute('width', w);
            this.svg.setAttribute('height', h);
            
            // Set viewBox to logical size
        ...
        /**
         * Override drawShape to render using SVG Context instead of WebGL
         * This is necessary because p5.js v2's Renderer2D uses WebGL for shape rendering
         */
        /**
         * Override drawShape to render using SVG Context instead of WebGL
         * This is necessary because p5.js v2's Renderer2D uses WebGL for shape rendering,
         * which doesn't work with SVG output.
         */
        RendererSVG.prototype.drawShape = function (shape) {
            // Only render if there's fill or stroke color
            ...
            // Determine shape mode from the shape's first contour
            // PATH constant is 0x0008 = 8
            var PATH_MODE = 8; // p5.PATH constant value
            var shapeMode = shape.contours && shape.contours.length > 0 ? shape.contours[0].kind : PATH_MODE;
            
            // For PATH mode (beginShape/endShape), use PrimitiveToVerticesConverter to extract vertices
            if (shapeMode === PATH_MODE) {
                // Use PrimitiveToVerticesConverter to convert shape primitives to vertices
                var visitor = new p5.PrimitiveToVerticesConverter({
                    curveDetail: this.states.curveDetail || 0.25
                });
                shape.accept(visitor);
                
                ctx.beginPath();
                
                // Process each contour from the visitor
                for (var c = 0; c < visitor.contours.length; c++) {
                    var contour = visitor.contours[c];
                    if (contour.length === 0) continue;
                    
                    // Check if this contour should be closed
                    // In p5.js v2, the original shape's contour has an isClosed property
                    // when endShape(CLOSE) was called
                    var isClosed = false;
                    if (shape.contours && shape.contours[c] && shape.contours[c].isClosed === true) {
                        isClosed = true;
                    }
                    
                    // Extract coordinates from each vertex
                    // Vertices from PrimitiveToVerticesConverter have a position property
                    var first = true;
                    var firstX, firstY;
                    
                    for (var v = 0; v < contour.length; v++) {
                        var vertex = contour[v];
                        var x, y;
                        
                        // Extract coordinates - vertices have position property with x, y
                        if (vertex && vertex.position) {
                            var pos = vertex.position;
                            if (pos.x !== undefined && pos.y !== undefined) {
                                x = pos.x;
                                y = pos.y;
                            } else if (pos.array && Array.isArray(pos.array) && pos.array.length >= 2) {
                                x = pos.array[0];
                                y = pos.array[1];
                            }
                        } else if (vertex && vertex.x !== undefined && vertex.y !== undefined) {
                            // Direct x, y properties (fallback)
                            x = vertex.x;
                            y = vertex.y;
                        } else if (Array.isArray(vertex) && vertex.length >= 2) {
                            // Array format [x, y, z] (fallback)
                            x = vertex[0];
                            y = vertex[1];
                        }
                    ...
                    // Only close path if the shape was explicitly closed with endShape(CLOSE)
                    if (!first && isClosed) {
                        ctx.closePath();
                    }
                }
                ...
                // Fall back to parent implementation for other shape modes
                p5.Renderer2D.prototype.drawShape.call(this, shape);
            }
        ...
        // For p5.js v2, register the SVG renderer
        if (isV2) {
            p5.registerAddon(function(p5, p5Instance, lifecycles) {
                p5.RendererSVG = p5.RendererSVG;
                p5.renderers['svg'] = p5.RendererSVG;
                p5Instance.SVG = 'svg';
            });
        }
    }
    ...
    function Rendering (p5, isV2) {
        ...
        p5.prototype.createCanvas = function (w, h, renderer) {
            if (renderer === constants.SVG) {
                if (isV2) {
                    // p5.js v2: Create a regular canvas first, then replace with SVG
                    var graphics = _createCanvas.apply(this, [w, h, 'p2d']);
                    var c = graphics.canvas;
                    this._renderer = new p5.RendererSVG(c, this, true);
                    this._isdefaultGraphics = true;
                    this._renderer.resize(w, h);
                    this._renderer._applyDefaults();
                } else {
                    // p5.js v1: createCanvas returns graphics with canvas property
                ...
                // For non-SVG renderers, use the original createCanvas
    ...
    function Image$1 (p5) {
        p5.prototype.loadPixels = function () {
            ...
            // Only call _validateParameters if it exists (p5.js v1 has it, v2 doesn't)
    ...
    function init(p5) {
        // Detect p5.js version - simple and reliable
        const isV2 = typeof p5.registerAddon === 'function';
        
        var p5svg = p5;
        RendererSVG(p5svg, isV2);
        SVGFiters(p5svg);
        Rendering(p5svg, isV2);
        IO(p5svg);
        Image$1(p5svg);
        Filters(p5svg);
        Element(p5svg);
